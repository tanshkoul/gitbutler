...
pub(crate) fn move_changes_between_commits(
    ctx: &CommandContext,
    ...
    for step in &mut source_stack_steps {
        ...
        let RebaseStep::Pick { new_tree, .. } = step else {
            continue;
        };
        *new_tree = Some(source_tree_without_changes_id);
    }
    ...
    if source_stack_id == destination_stack_id {
        ...
        for step in &mut source_stack_steps {
            ...
            *commit_id = *source_stack_mapping.get(commit_id).unwrap_or(commit_id);
            ...
            if *commit_id == destination_commit_id {
                *new_tree = Some(final_destination_tree.detach());
            }
        }
        ...
        let destination_stack = vb_state.get_stack_in_workspace(source_stack_id)?;
